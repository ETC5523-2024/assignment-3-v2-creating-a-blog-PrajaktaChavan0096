...
First, we load the datasets using the following code:
...
```{r}
# Check the column names to ensure you're selecting the correct ones
colnames(high_food_2020)
...
Food Expenditure: Wealthier nations, such as the United States and Switzerland, spend significantly more on food per person compared to lower-income nations (see Figure 1 and Table 1).
High Food Prices: 2020 saw a significant spike in countries experiencing abnormally high food prices (see Figure 2), underscoring the vulnerability of food markets during global crises.
